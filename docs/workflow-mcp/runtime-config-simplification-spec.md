...
**Post-cutover:** verify scheduler activation, a genuine schedule occurrence, regular MCP/connection behavior, and a deliberately invoked heavy GitHub/R2 acceptance run **after** the deployment job ended. Only once the code no longer references redundant secrets and all checks pass, explicitly delete superseded production smoke bindings and obsolete dashboard overrides; verify those names remain absent after the next deployment.
...
| C04 | Inspect generated Wrangler config and Worker schedule | The configured minute Cron survives generation/deployment; a real scheduled workflow can be admitted after CI has ended. |
