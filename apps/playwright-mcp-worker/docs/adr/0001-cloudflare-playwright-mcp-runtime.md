...
Cloudflare's official Playwright MCP implementation is designed for Workers + Browser Run and currently requires a Browser binding plus an `McpAgent` Durable Object with migration metadata. This is materially different from the stateless transport used by several existing apps, so the exception must be explicit rather than accidental.

Cloudflare's current Playwright MCP documentation still demonstrates `createMcpAgent()` and the Durable Object runtime. Separately, the newer general Agents MCP documentation marks `McpAgent` as deprecated/feature-frozen and recommends `createMcpHandler()` for new stateless MCP servers. The official `@cloudflare/playwright-mcp` source still subclasses `McpAgent`, so removing that dependency today would require maintaining a local adaptation instead of using the supported Playwright MCP wrapper.
...
- Accept the upstream-required `McpAgent`/Durable Object runtime as a narrowly scoped compatibility exception to the repository's stateless preference. Do not build a custom stateless Playwright MCP adapter in V1 merely to eliminate this upstream dependency.
- Revisit that exception when Cloudflare publishes an official Playwright MCP wrapper based on the current stateless MCP runtime, or when the legacy `McpAgent` path becomes operationally incompatible.
...
## Consequences
The app stays consistent with `mcp-workers` operational patterns while using the stateful runtime Cloudflare's official Playwright MCP currently requires. The public attack surface is smaller than the official example because only `/mcp` is exposed and it is authenticated from day one.

V1 remains intentionally general-purpose and low-maintenance. It can navigate, inspect, and interact using the official tool surface without introducing a local fork. The trade-off is that this app temporarily carries an upstream legacy-runtime dependency even though the repository otherwise prefers stateless MCP transport. This debt is bounded: it is isolated to this app and has an explicit migration trigger.

Per-destination browser restrictions and persistent authenticated browser profiles are deferred; those require separate design decisions if needed later.
