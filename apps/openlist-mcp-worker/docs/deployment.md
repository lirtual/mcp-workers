...
3. Configure `OPENLIST_URL`, `OPENLIST_ALLOWED_PATHS`, and any optional read-only/upload/timeout settings.
4. Store `OPENLIST_TOKEN` as the Worker-to-OpenList business secret.
5. Store a separate high-entropy `MCP_ACCESS_TOKEN` for Portal-to-Worker authentication.
...
`OPENLIST_TOKEN`, path allowlists, read-only mode, destructive-operation confirmation, upload bounds, and timeouts remain separate controls.
...
7. Verify one allowed and one denied path.
...
Only enable writes after the read-only path and path restrictions have been verified.
