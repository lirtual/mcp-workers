...
describe('Raindrop daily snapshot', () => {
  ...
  it('uses one immutable endpoint, shared ingress credential, and explicit local read policy', () => {
    ...
    expect(connection?.auth.secret).toBe('MCP_ACCESS_TOKEN');
    expect(findWorkflow('raindrop-daily-snapshot')?.definitionDigest).toBe(compiled.definitionDigest);
  ...
  it.each([0, 4, 20])('invokes the read-only MCP adapter and preserves %i actual records', async count => {
    const items = Array.from({ length: count }, (_, n) => ({ id: n + 1, title: 'Bookmark ' + n, url: 'https://example.test/' + n, tags: [] }));
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const result = body.method === 'tools/list'
        ? { tools: [{ name: 'list_raindrops', outputSchema: {
            type: 'object', required: ['items', 'count'], properties: {
              items: { type: 'array', items: { type: 'object', required: ['id', 'title', 'url', 'tags'], properties: {
    ...
    const result = await callMcpTool(
      { MCP_ACCESS_TOKEN: 'raindrop-secret' } as Env,
      'raindrop',
      'list_raindrops',
  ...
  it('keeps execution local and returns records through the compiled workflow output', async () => {
    const items = [{ id: 42, title: 'Snapshot', url: 'https://example.test/42', tags: [] }];
    const methods: string[] = [];
    vi.stubGlobal('fetch', vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      const body = JSON.parse(String(init?.body)) as Record<string, unknown>;
      methods.push(String(body.method));
      const response = body.method === 'tools/list'
        ? { tools: [{ name: 'list_raindrops', inputSchema: { type: 'object',
            required: ['collectionId'], properties: { collectionId: { type: 'number' } }
          }, outputSchema: { type: 'object', required: ['items', 'count'], properties: {
            items: { type: 'array' }, count: { type: 'number' }
          } } }] }
        : { structuredContent: { items, count: 1 }, content: [] };
      return Response.json({ jsonrpc: '2.0', id: body.id, result: response });
    }));
    const step = plan.steps.fetch!;
    const invocation = await executeCloudflareCapability(step.uses, step.with, {
      env: { MCP_ACCESS_TOKEN: 'test-token' } as Env,
      operationId: 'op-snapshot',
      effectivePolicy: resolveMcpOperationPolicy('raindrop', 'list_raindrops', undefined)
    });
    expect(invocation.state).toBe('succeeded');
    if (invocation.state !== 'succeeded') throw new Error('Expected local invocation success');
    expect(resolveRuntimeValue(plan.outputs, { input: {}, stepOutputs: { fetch: invocation.output } }))
      .toEqual({ bookmarks: items, count: 1 });
    expect(methods).toEqual(['tools/list', 'tools/call']);
  });

  it('rejects malformed structured output instead of silently succeeding', async () => {
    const fetchImpl = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      const body = JSON.parse(String(init?.body)) as Record<string, unknown>;
      const response = body.method === 'tools/list'
        ...
        : { structuredContent: { count: 0 }, content: [] };
      return Response.json({ jsonrpc: '2.0', id: body.id, result: response });
    });
    await expect(callMcpTool({ MCP_ACCESS_TOKEN: 'token' } as Env,
      'raindrop', 'list_raindrops', {}, fetchImpl as typeof fetch
    )).rejects.toThrow('MCP_OUTPUT_SCHEMA_MISMATCH');
  });
