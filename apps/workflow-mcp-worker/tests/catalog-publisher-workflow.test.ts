...
describe('T09 trusted Engine publisher workflow', () => {
  it('runs only as a protected manual, isolated definition-only publisher', () => {
    ...
    expect(job.environment).toBe('workflow-mcp-publisher-isolated');
  ...
  it('requires the explicitly approved source SHA and a safe default dry-run mode', () => {
    ...
    expect(workflow).toContain('environment: workflow-mcp-publisher-isolated');
  });
});
