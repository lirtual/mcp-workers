 ...
 *
 * Resolution priority:
 *   1. Normalize --content-type by removing parameters and lowercasing it
 *   2. If --content-type is provided and recognized → use it (content-type rules over extension)
 *      application/zip is accepted only when the filename ends in .xmind
 *   3. If --content-type is unrecognized, fall back to extension
 *   4. If no --content-type, use extension
 *   5. If neither can resolve → fail
 *
 * Usage:
...
const inputContentType = normalizeContentType(args['content-type'] || '');
...
if (inputContentType === 'application/zip') {
  if (ext === 'xmind') {
    ...
    fail({
      ...base,
      reason: 'Generic ZIP files are not supported. application/zip is accepted only for files with the .xmind extension.',
    });
  }
