...
function loadCredentials(options = {}) {
  ...
  if (!clientId || !apiKey) {
    ...
    err.msg =
      '未找到 IMA 凭证（Client ID / API Key）。请设置 IMA_OPENAPI_CLIENTID 和 IMA_OPENAPI_APIKEY，或将凭证放置在 ~/.config/ima/client_id 和 ~/.config/ima/api_key。';
    throw err;
  }
...
async function checkSkillUpdateIfNeeded(requestOptions) {
  ...
  if (isNewerVersion(latestVersion, skillVersion)) {
    const updateContext = {
