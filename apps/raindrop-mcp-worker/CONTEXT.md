...
**Unresolved final-consensus dependencies**: specific read-only group names, schema/version compatibility and operation-to-group mapping; comparative prototype's native CPU/control criteria and build/maintenance rubric; exact MCP protocol fixtures and ChatGPT Portal acceptance; predeclared sample counts, persistence/overage definition, missing data and error rule, plus resource gate for writes after the read-only prototype; v4 isolation/workflow identity, credentials, artifact handling and rollback; conditional migration and v3 PR disposition. The first three rounds are architecture consensus, **not to-spec approval**. No production changes or implementation are authorized by this record.
