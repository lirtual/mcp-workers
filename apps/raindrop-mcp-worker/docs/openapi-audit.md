...
At the audit baseline, the historically named `raindrop-complete.yaml` declared 43 route keys. The v3 runtime uses **16** documented route shapes (some with multiple HTTP methods). The historical source lacked two route keys and global tag methods. **Resolved in the PR branch:** the active YAML now contains exactly those 16 keys; the generated `raindrop.schema.d.ts` was regenerated from it. The table below preserves the *before* findings for review history, not the present state. Removing inactive paths does not assert other official Raindrop endpoints are nonexistent.

| Scope | Required method and exact path | Audit baseline (before fix) |
| --- | --- | --- |
...
The following 29 historical keys were *not* called by the 26 v3 tools and are now removed from the active generation source. The first group also contains known mismatches with the current official endpoint mapping; do not retain it to imply support:
...
**Current state:** 16 active route keys are committed in `raindrop-complete.yaml`; the corresponding declaration was regenerated by [Schema Sync #35515372464](https://github.com/lirtual/mcp-workers/actions/runs/35515372464). The active collection, bookmark, highlight, tag and Trash requests in `raindrop.service.ts` are typed against `openapi-fetch<paths>` rather than `(this.client as any)`; required/default fields and official tag write bodies are aligned with observed isolated live behavior. `check:schema` passed in the application check of [CI #35515414295](https://github.com/lirtual/mcp-workers/actions/runs/35515414295). Historical unused **component schemas** remain for now; their presence does not imply that v3 implements them. This audit does not certify complete official API coverage or validate production rollout.
