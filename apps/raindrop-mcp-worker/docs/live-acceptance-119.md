...
- The existing `.github/workflows/raindrop-v3-test.yml` **still performs test-owned account writes following the read-only smoke**. An attempt to add an explicit read-only execution mode was blocked by the integration's workflow safety check. Do **not** manually dispatch that workflow just to gather CPU samples, assume it is safe by default, or work around that check. The optional `RAINDROP_V3_CPU_PROFILE=true` script path has not executed on Cloudflare.
...
**Next safe measurement path:** obtain an approved read-only-only dispatch of the existing isolated workflow (without its mutation suites), pin the deployed SHA and Cloudflare script version, capture repeated native `$workers.cpuTimeMs` for fixed operation labels, and separately establish Free entitlement/enforcement with appropriately authorized account information. Never infer function-level hot spots from request-level CPU alone.
