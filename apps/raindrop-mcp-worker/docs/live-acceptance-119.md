...
## 2026-09-21 follow-up: CI, isolated deployment, and Free-plan verification boundary

- PR HEAD `19e34d18385a344c2aa93148f4efaca084a1c277`: [CI #35521579174](https://github.com/lirtual/mcp-workers/actions/runs/35521579174) has completed successfully, including application checks and database integration. This validates repository CI, **not** CPU acceptance.
- At the time of this follow-up, Cloudflare's deployment-list API still showed isolated Worker `raindrop-mcp-worker-v3-test` serving prior version `cb3b4b7a-504f-4571-974b-b18227c6a1c7` at 100%. No deployment of the newer probe/HEAD is documented. Production was not changed.
- At the time of this historical follow-up, the then-current `.github/workflows/raindrop-v3-test.yml` **still performed test-owned account writes following the read-only smoke**. An attempt to add an explicit read-only mode was blocked by the integration's workflow safety check. Do not run that older mutation-capable workflow for CPU profiling. Its prior status is not a description of the latest PR source (see correction below).
- The Cloudflare account settings endpoint returned `default_usage_model=standard`; the account subscriptions endpoint returned authentication error `10000` under the connected credential. Thus this probe cannot verify billing entitlement, true Free-plan execution, or a 10 ms enforcement test. The absence of runtime errors on a Standard-model invocation is not Free acceptance.
- Latest PR review lookup returned **no reviews**. Independent #118 Standards/Spec sign-off remains outstanding.

**Next safe measurement path:** obtain separate authorization for a source-pinned, read-only-only isolated probe, correlate deployed SHA with Cloudflare script version, capture repeated native `$workers.cpuTimeMs` for fixed operation labels, and separately establish Free entitlement/enforcement with appropriately authorized account information. Never run an older mutation-enabled workflow or infer function-level hot spots from request-level CPU alone.

## 2026-09-21 current-source correction (not a live acceptance)

At source commit `c1a6222b57c0c84f7bd7f3a7d483fe462b110ca2`, `.github/workflows/raindrop-v3-test.yml:3–9,89–99` is restricted to a push on the v3 branch that changes the workflow file. Its current command runs `test-direct-mcp.mjs` with `RAINDROP_V3_CPU_PROFILE=true` after uploading to the **isolated** Worker; the previous mutation lifecycle suites are no longer invoked by this workflow. The #118 repair commits did not change that workflow path and did not trigger the isolated deploy. This is source inspection only, not a claim that the current-source probe has run, a Portal verification, or Free-plan acceptance.

The repair added offline tests for the read-only suggestion POST error classification, required 26-tool output envelopes and authenticated `worker.fetch` read/write `tools/call` via fake upstream. [Application CI #35589576376](https://github.com/lirtual/mcp-workers/actions/runs/35589576376) passed 211 tests with 3 skipped and 26 lint warnings/0 errors. This does not satisfy independent #118 sign-off or #119/#128 live evidence gates.
