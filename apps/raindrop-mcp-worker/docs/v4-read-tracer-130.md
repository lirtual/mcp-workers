...
- `tests/v4_read_tracer.test.ts` pins the deterministic, synthetic protocol/input and upstream-response fixtures. Its local SDK `Client + InMemoryTransport` tests cover initialization, 28-tool discovery, annotations and strict schemas; the authenticated `worker.fetch` tests cover initialize, tools/list, both tool calls, resources/list/read, prompts/list/get, unauthorized and oversized ingress, invalid action, and in-flight disconnect. The disconnect fixture verifies that the upstream request's actual signal becomes aborted, no second upstream request is started, and no successful result is returned. Stateless HTTP may legitimately terminate with an empty body after disconnect; an emitted MCP payload must contain an error envelope. The list fixture uses only a synthetic single bookmark and bounded `perpage=1`. A read-only upstream 401 fixture is compared directly to the unchanged v3 envelope. Existing v3 contract and ingress/cancellation suites are also retained.
- GitHub Actions `CI` on implementation commit `b2dff3a85225d63f10247e9ed3b4ab58511a89f4`: [#35833698136](https://github.com/lirtual/mcp-workers/actions/runs/35833698136), SUCCESS: TypeScript; lint 0 errors, 26 existing warnings; 20 Vitest files / 222 passed / 3 skipped; 14/14 CPU harness unit tests; schema regeneration and Wrangler dry-run. This source is not an immutable deployment identity until the final Draft PR #141 SHA and Cloudflare version are correlated. Subsequent documentation-only commits require their own exact-SHA CI evidence.
